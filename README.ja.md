...
Gemini に明示します。さらに temperature を 1.3 にして、毎回ちゃんと
異なる方向性が出るようにしています。プロンプト自体も意図的に、
**Gemini に「自明でない角度」を探させる**書き方に直してあります
(珍しいターゲットユーザー、メタファー、異分野マッシュアップ、
制約を逆手に取った特徴化など)。カードを字面どおりに組み合わせた
最も直接的な成果物で終わらせません。

**追加の引導(任意)**: ガチャボタンの上に小さな入力欄(音声入力
対応)があり、「高齢者向けに」「絶対に電源を使わないおもちゃに」
「儀式感を足して」など、今回の引き方の方向性を自然言語で Gemini
に伝えられます。空欄ならデフォルトのクリエイティブプロンプトで
生成され、書き込めば**強い方向制約**としてプロンプトに注入される
ので、出力のトーンが大きく変わります。入力内容は proposal に
保存され、履歴を見直したときに「🎯 你的引導」として表示されるので、
「これは当時どう steer して生まれた案か」が後から確認できます。
入力欄は生成成功のたびにクリアされるので、次のラウンドは常に
まっさらから始められます。
